...
{ 
  ...
  rf <- RRF(X.train,Y.train, flagReg = 0, importance = TRUE)
  impRF <- rf$importance
  ...
  grrf <- RRF(X.train, Y.train, coefReg=coefReg, flagReg=1, importance = TRUE)
  
  # Selected set of features used for checking prediction accuracy
  a <- grrf$feaSet
  selected <- length(a)
  selected1 <- sum(ifelse(a <= m/10 , 1, 0))
  # when Not regularized this may happen
  
  #rf2 <-RRF(X.train,Y.train, flagReg = 1, importance = TRUE)
  # rf2$feaSet
  ...
  return(list(errors,a))
} 

rrf_simulation(20000,500)
